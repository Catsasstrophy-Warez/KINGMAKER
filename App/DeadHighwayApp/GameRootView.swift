...
struct TouchControls: View { @Bindable var session:DeadHighwaySession; var body:some View { HStack { VStack{Button("▲"){};HStack{Button("◀"){};Button("▼"){};Button("▶"){} }}.buttonStyle(.borderedProminent); Spacer(); VStack{Button("INSPECT"){session.inspectKingmaker()}; Button("DIAGNOSE"){session.diagnoseKingmaker()}; Button("REPAIR"){session.repairKingmaker()}; Button("ADVANCE \(session.slice.beat.rawValue.uppercased())"){session.advanceSlice()}; HStack{Button("SAVE"){session.saveSlice()};Button("RELOAD"){session.reloadSlice()}}; Button(session.inVehicle ? "BRAKE":"SPRINT"){} }.buttonStyle(.borderedProminent) } } }
